...
type MessageQuery struct {
	...
	// eager-loading edges.
	withFromChat   *ChatQuery
	withFromSender *SenderQuery
...
// QueryFromChat chains the current query on the "from_chat" edge.
func (mq *MessageQuery) QueryFromChat() *ChatQuery {
...
// QueryFromSender chains the current query on the "from_sender" edge.
func (mq *MessageQuery) QueryFromSender() *SenderQuery {
	query := &SenderQuery{config: mq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := mq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (mq *MessageQuery) Clone() *MessageQuery {
	...
	return &MessageQuery{
		...
		withFromChat:   mq.withFromChat.Clone(),
		withFromSender: mq.withFromSender.Clone(),
...
// WithFromChat tells the query-builder to eager-load the nodes that are connected to
// the "from_chat" edge. The optional arguments are used to configure the query builder of the edge.
func (mq *MessageQuery) WithFromChat(opts ...func(*ChatQuery)) *MessageQuery {
	query := &ChatQuery{config: mq.config}
	for _, opt := range opts {
		opt(query)
	}
	mq.withFromChat = query
	return mq
}

// WithFromSender tells the query-builder to eager-load the nodes that are connected to
// the "from_sender" edge. The optional arguments are used to configure the query builder of the edge.
func (mq *MessageQuery) WithFromSender(opts ...func(*SenderQuery)) *MessageQuery {
	query := &SenderQuery{config: mq.config}
	for _, opt := range opts {
		opt(query)
	}
	mq.withFromSender = query
	return mq
}
...
func (mq *MessageQuery) sqlAll(ctx context.Context) ([]*Message, error) {
	...
	if query := mq.withFromChat; query != nil {
		...
		for _, n := range neighbors {
			...
			for i := range nodes {
				nodes[i].Edges.FromChat = n
			}
		}
	}

	if query := mq.withFromSender; query != nil {
		ids := make([]int64, 0, len(nodes))
		nodeids := make(map[int64][]*Message)
		for i := range nodes {
